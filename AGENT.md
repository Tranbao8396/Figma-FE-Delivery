...
| `context-builder/` | Unified `figma-context` CLI, contract tests, intake template and guide | Reusable, no customer data |
| `design-collector/` | Internal Figma REST/JSON collector module and guide | Reusable, no customer data |
...
- `figma-context` is the sole user-facing CLI. Its `prepare` command orchestrates Collector, Normalizer and Builder; modules remain separate internally for test, cache and network/token boundaries.
