...
function buildSourceContext(inventory, options = {}) {
  ...
  const context = {
    ...
    topology: { entryCandidates: files.filter((file) => /(^|\/)(src\/)?(main|index|app)\.[cm]?[jt]sx?$/i.test(file)).slice(0, 12), configCandidates: files.filter((file) => /(^|\/)(package\.json|vite\.config|next\.config|webpack\.config|tsconfig|eslint|prettier)/i.test(file)).slice(0, 16) },
