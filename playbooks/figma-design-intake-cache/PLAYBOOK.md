...
9. For implementation, read the phase record from `project-context-index.json`, then use only its named project artifacts. For review/QC, read those project artifacts first and then the task-local evidence artifacts named by approved `reportRefs`. Re-open Figma web or call MCP only after an invalidation trigger through this playbook.
